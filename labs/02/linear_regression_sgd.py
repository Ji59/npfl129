...
def main(args):
	...
	x = np.c_[data, np.ones((len(data), 1))]
	...
	for epoch in range(args.epochs):
		...
		for i in range(int(train_data.shape[0] / args.batch_size)):
			...
			for j in range(args.batch_size):
				data_temp = train_data[permutation[i * args.batch_size + j]]
				target_temp = train_target[permutation[i * args.batch_size + j]]
				gradient = (np.transpose(data_temp) @ weights - target_temp) * data_temp
				gradient_sum += gradient
			weights = weights - args.learning_rate * (gradient_sum / args.batch_size)

		# TODO: Append current RMSE on train/test to train_rmses/test_rmses.

		train_rmse = np.sqrt(np.sum(np.power(train_data @ weights - train_target, 2)) / len(train_target))
		train_rmses.append(train_rmse)
		test_rmse = np.sqrt(np.sum(np.power(test_data @ weights - test_target, 2)) / len(test_target))
	...
	explicit_rmse = np.sqrt(np.sum(np.power(test_target - y_predicted, 2)) / len(test_target))
