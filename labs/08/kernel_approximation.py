...
class RFFsTransformer(sklearn.base.TransformerMixin):
	...
	def fit(self, X, y=None):
		...
		self._w = np.sqrt(2 * self._gamma) * generator.normal(0, 1, (X.shape[1], self._n_components))
		self._b = generator.uniform(0, 2 * np.pi, self._n_components)

		return self

	def transform(self, X):
		# TODO: Transform the given `X` using precomputed `w` and `b`.
		return np.sqrt(2 / len(self._b)) * np.cos(X @ self._w + self._b)
...
class NystroemTransformer(sklearn.base.TransformerMixin):
	...
	def _rbf_kernel(self, X, Z):
		...
		arrays = []
		for x in X:
			arrays.append(np.exp(-self._gamma * np.linalg.norm(Z - x, axis=1) ** 2))
	...
	def transform(self, X):
		...
		kernel = self._rbf_kernel(X, self._samples)

		return kernel @ self._v
