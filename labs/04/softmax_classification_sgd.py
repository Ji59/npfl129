	...
	for iteration in range(args.iterations):
		...
		for i in permutation:
			...
			for j in range(weights.shape[1]):
				w = weights[:, j]
				y[j] = np.transpose(x) @ w
			y -= y.max()
			for j in range(weights.shape[1]):
				g[:, j] += (np.exp(y[j]) / np.sum(np.exp(y)) - (1 if j == train_target[i] else 0)) * x
		...
		train_accuracy = np.sum([1 if train_target[i] == np.argmax(np.transpose(train_data[i]) @ weights) else 0 for i in range(train_data.shape[0])]) / train_data.shape[0]
		...
		for i in range(train_data.shape[0]):
			...
			for j in range(weights.shape[1]):
				...
				y[j] = np.transpose(x) @ w
			y -= y.max()
			ti = np.exp(y[train_target[i]])
			train_loss -= np.log(ti / np.sum(np.exp(y)))
		train_loss /= train_data.shape[0]

		test_accuracy = np.sum([1 if (test_target[i] == np.argmax(np.transpose(test_data[i]) @ weights)) else 0 for i in range(test_data.shape[0])]) / test_data.shape[0]
		...
		for i in range(test_data.shape[0]):
			...
			for j in range(weights.shape[1]):
				...
				y[j] = np.transpose(x) @ w
			y -= y.max()
			ti = np.exp(y[test_target[i]])
			test_loss -= np.log(ti / np.sum(np.exp(y)))
