...
def main(args):
	...
	def forward(inputs):
		values = inputs @ weights[0] + biases[0]
		hidden = [value if value > 0 else 0 for value in values]
		yin = hidden @ weights[1] + biases[1]
		yin -= max(yin)
		output = [np.exp(y_value) / np.sum(np.exp(yin)) for y_value in yin]
	...
	for iteration in range(args.iterations):
		...
		gw0 = np.zeros(weights[0].shape)
		gw1 = np.zeros(weights[1].shape)
		gb0 = np.zeros(biases[0].shape)
		gb1 = np.zeros(biases[1].shape)
		...
		for i in permutation:
			...
			dL_dYin = y - np.array([1 if j == train_target[i] else 0 for j in range(len(y))])
			dL_dWy = np.zeros(weights[1].shape)
			for jj in range(len(y)):
				for ii in range(len(h)):
					dL_dWy[ii, jj] = dL_dYin[jj] * h[ii]
			dL_dby = dL_dYin

			dL_dh = weights[1] @ dL_dYin
			dL_dhin = dL_dh * [hh > 0 for hh in h]
			dL_dWh = np.matmul(dL_dhin.reshape(len(dL_dhin), 1), x.reshape(1, len(x)))
			dL_dbh = dL_dhin

			gw0 += np.transpose(dL_dWh)
			gw1 += dL_dWy
			gb0 += dL_dbh
			gb1 += dL_dby
		...
		for i in range(train_data.shape[0]):
			x = train_data[i]
			h = np.zeros(weights[0].shape[1])
			for j in range(weights[0].shape[1]):
				h[j] = max(0, x @ weights[0][:, j] + biases[0][j])
			train_accuracy += 1 if train_target[i] == np.argmax(h @ weights[1]) else 0
		...
		for i in range(test_data.shape[0]):
			...
			for j in range(weights[0].shape[1]):
				h[j] = max(0, x @ weights[0][:, j] + biases[0][j])
			test_accuracy += 1 if test_target[i] == np.argmax(h @ weights[1]) else 0
