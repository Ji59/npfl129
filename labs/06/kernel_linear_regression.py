...
def get_gaussian_kernel_xy(x, y):
	diff = x - y;
	return np.exp(-gamma * diff @ diff);


def get_kernel(data0, data1):
	same = np.all(data0 == data1)
	if (len(data0.shape) < 2):
		data0 = data0.reshape(-1, 1)
	if (len(data1.shape) < 2):
		data1 = data1.reshape(-1, 1)
	train_kernel = np.zeros((data0.shape[0], data1.shape[0]));
	for i in range(len(data0)):
		x = np.array(data0[i]);
		if same:
			for j in range(i, len(data1)):
				y = np.array(data1[j]);
				train_kernel[j, i] = train_kernel[i, j] = (get_gaussian_kernel_xy(x, y) if (gaussian) else get_poly_kernel_xy(x, y));
		else:
			for j in range(len(data1)):
				y = np.array(data1[j]);
				train_kernel[i, j] = (get_gaussian_kernel_xy(x, y) if (gaussian) else get_poly_kernel_xy(x, y));
	return train_kernel;


def get_rmse(kernel, target, beta, bias):
	mse = 0;
	for i in range(len(target)):
		error = target[i] - np.sum(kernel[i] @ beta) - bias;
		mse += error * error;
	return np.sqrt(mse / len(target));
...
def main(args):
	...
	betas_temp = np.zeros(batch_size);
	...
	train_kernel = get_kernel(train_data, train_data);
	test_kernel = get_kernel(test_data, train_data);
	...
	for iteration in range(args.iterations):
		...
		for i in range(len(permutation)):
			j = i % batch_size;
			c = permutation[i]
			betas_temp[j] = args.learning_rate * (train_targets[c] - train_kernel[c] @ betas - bias) / batch_size;

			if (j + 1 == batch_size):
				betas = (1 - args.learning_rate * args.l2) * betas;
				for k in range(batch_size):
					betas[permutation[i - k]] += betas_temp[-k - 1];


		# TODO: Append RMSE on training and testing data to `train_rmses` and
		# `test_rmses` after the iteration.

		train_rmses.append(get_rmse(train_kernel, train_targets, betas, bias));
		test_rmses.append(get_rmse(test_kernel, test_targets, betas, bias));
