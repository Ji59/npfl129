...
def distance(y, x, t, p):
	return np.c_[np.power(np.sum(np.abs(power(x - y, p)), axis=1), 1 / p), t]


def softmax(x):
	expsum = np.sum(np.exp([-i[0] for i in x]))
	return [np.exp(-xi[0]) / expsum for xi in x]


def get_k_nearest(in_list, k):
	temp = list(in_list[0:k])
	temp.sort(key=lambda l: l[0])
	k_nearest = np.array(temp)
	for x in in_list[k:]:
		value = x[0]
		if value < k_nearest[-1][0]:
			j = 0
			for i in range(len(k_nearest) - 2, -1, -1):
				if k_nearest[i][0] <= value:
					j = i + 1
					break
			k_nearest = np.insert(k_nearest, j, x, axis=0)[:k]
	return k_nearest
...
def main(args):
	...
	for test in test_data:
		distance_list = distance(test, train_data, train_target, args.p)
		# for i in range(len(train_data)):
		# 	dist = distance(test, train_data[i], args.p)
		# 	distance_list.append((dist, train_target[i]))
		k_nearest = get_k_nearest(distance_list, args.k)
		# k_nearest = distance_list[:args.k]
		weights = np.ones(args.k) if args.weights == "uniform" else \
			np.array([1 / w[0] for w in k_nearest]) if args.weights == "inverse" else \
			np.array(softmax(k_nearest))
		t = np.zeros(10)
		weights_sum = np.sum(weights)
		for i in range(args.k):
			t[int(k_nearest[i][1])] += weights[i] / weights_sum
		test_predictions = np.append(test_predictions, np.argmax(t))
